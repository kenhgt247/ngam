...
test.describe.serial('DHHD v0.14 hidden qpdf web optimization real-file gate',()=>{
  ...
  test('83 web optimizer stays hidden from 53-tool public home',async({page})=>{await page.goto(`${BASE}/#home`,{waitUntil:'load'});await page.waitForFunction(()=>document.querySelectorAll('[data-tool-card]').length===53,null,{timeout:30000});await expect(page.locator('[href="#weboptimize"]')).toHaveCount(0);await expect(page.locator('.hero .eyebrow')).toHaveText('DHHD TOOLS v0.11.0')});
  test('84 mobile WebKit runs qpdf WASM and produces linearized output',async()=>{const browser=await webkit.launch(),ctx=await browser.newContext({...devices['iPhone 13']}),page=await ctx.newPage();const p=await runOptimize(page,'84-webkit-linearized.pdf');await validate(p);await ctx.close();await browser.close()});
});
