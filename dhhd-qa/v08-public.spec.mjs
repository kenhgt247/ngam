...
test.describe.serial('DHHD v0.8 public regression on v0.11 production',()=>{
  test('53 v0.8 tools remain on 53-tool public home',async({page})=>{
    ...
    await expect(page.locator('.hero .eyebrow')).toHaveText('DHHD TOOLS v0.11.0');
    await expect(page.getByText('53 công cụ hoạt động')).toBeVisible();
    for(const key of v08)await expect(page.locator(`[href="#${key}"]`)).toHaveCount(1);
  });

  test('54 static fallback keeps v0.8 tools inside 53 cards',async()=>{
    ...
    await expect(page.locator('.hero .eyebrow')).toHaveText('DHHD TOOLS v0.11.0');
    await expect(page.locator('.grid .card')).toHaveCount(53);
    for(const key of v08)await expect(page.locator(`[href="#${key}"]`)).toHaveCount(1);
    await ctx.close();await browser.close();
  });

  test('55 mobile WebKit still opens all v0.8 routes on v0.11',async()=>{
    const browser=await webkit.launch(),ctx=await browser.newContext({...devices['iPhone 13']}),page=await ctx.newPage();
    await page.goto(`${BASE}/#home`,{waitUntil:'load'});await page.waitForFunction(()=>document.querySelectorAll('[data-tool-card]').length===53,null,{timeout:30000});
    for(const key of v08)await expect(page.locator(`[href="#${key}"]`)).toHaveCount(1);
