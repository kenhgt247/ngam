...
test.describe.serial('DHHD v0.8 public regression on v0.14 production',()=>{
  test('53 v0.8 tools remain on 54-tool public home',async({page})=>{
    ...
    await expect(page.locator('.hero .eyebrow')).toHaveText('DHHD TOOLS v0.14.0');
    await expect(page.getByText('54 công cụ hoạt động')).toBeVisible();
    for(const key of v08)await expect(page.locator(`[href="#${key}"]`)).toHaveCount(1);
  });

  test('54 static fallback keeps v0.8 tools inside 54 cards',async()=>{
    ...
    await expect(page.locator('.hero .eyebrow')).toHaveText('DHHD TOOLS v0.14.0');
    await expect(page.locator('.grid .card')).toHaveCount(54);
    for(const key of v08)await expect(page.locator(`[href="#${key}"]`)).toHaveCount(1);
    await ctx.close();await browser.close();
  });

  test('55 mobile WebKit still opens all v0.8 routes on v0.14',async()=>{
    const browser=await webkit.launch(),ctx=await browser.newContext({...devices['iPhone 13']}),page=await ctx.newPage();
    await page.goto(`${BASE}/#home`,{waitUntil:'load'});await page.waitForFunction(()=>document.querySelectorAll('[data-tool-card]').length===54,null,{timeout:30000});
    for(const key of v08)await expect(page.locator(`[href="#${key}"]`)).toHaveCount(1);
