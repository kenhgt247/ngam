...
test.describe.serial('DHHD v0.6 public production real-file gate',()=>{
 ...
 test('39 public home exposes all six v0.6 tools and 35 total',async({page})=>{await page.goto(`${BASE}/#home`);await page.waitForFunction(()=>document.querySelectorAll('[data-tool-card]').length===35);await expect(page.getByText('35 công cụ hoạt động')).toBeVisible();for(const key of ['comparepdf','overlaypdf','extractimages','autocrop','imagewatermark','enhancescan'])await expect(page.locator(`[href="#${key}"]`)).toHaveCount(1)});
});
