...
test.describe.serial('DHHD v0.6 regression production real-file gate',()=>{
 ...
 test('39 all six v0.6 tools remain public on v0.7 home',async({page})=>{await page.goto(`${BASE}/#home`);await page.waitForFunction(()=>document.querySelectorAll('[data-tool-card]').length===41);for(const key of ['comparepdf','overlaypdf','extractimages','autocrop','imagewatermark','enhancescan'])await expect(page.locator(`[href="#${key}"]`)).toHaveCount(1)});
});
