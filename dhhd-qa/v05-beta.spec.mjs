...
test.describe.serial('DHHD v0.5 public real-file gate',()=>{
 ...
 test('30 all six v0.5 tools are public on home after promotion',async({page})=>{await page.goto(`${BASE}/#home`);await page.waitForFunction(()=>document.querySelectorAll('[data-tool-card]').length===29);await expect(page.locator('.hero .eyebrow')).toHaveText('DHHD TOOLS v0.5.0');await expect(page.locator('.heroBox strong')).toContainText('29 công cụ');for(const title of ['PDF → Text','OCR tài liệu','Xóa trang trắng','PDF đen trắng','Chuẩn hóa kích thước PDF','Repair / Rebuild PDF'])await expect(page.getByText(title,{exact:true})).toBeVisible()});
});
