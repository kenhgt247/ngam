...
test.describe.serial('DHHD v0.5 regression real-file gate',()=>{
 ...
 test('30 all six v0.5 tools remain public after v0.7 promotion',async({page})=>{await page.goto(`${BASE}/#home`);await page.waitForFunction(()=>document.querySelectorAll('[data-tool-card]').length===41);for(const title of ['PDF → Text','OCR tài liệu','Xóa trang trắng','PDF đen trắng','Chuẩn hóa kích thước PDF','Repair / Rebuild PDF'])await expect(page.getByText(title,{exact:true})).toBeVisible()});
});
