...
test.describe.serial('DHHD v0.5 hidden beta real-file gate',()=>{
 ...
 test('30 beta tools remain hidden from public home until gate passes',async({page})=>{await page.goto(`${BASE}/#home`);await page.waitForFunction(()=>document.querySelectorAll('[data-tool-card]').length===23);for(const title of ['PDF → Text','OCR tài liệu','Xóa trang trắng','PDF đen trắng','Chuẩn hóa kích thước PDF','Repair / Rebuild PDF'])await expect(page.getByText(title,{exact:true})).toHaveCount(0)});
});
