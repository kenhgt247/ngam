...
test.describe.serial('DHHD v0.9 production real-file gate',()=>{
  ...
  test('59 v0.9 remains public on 53-tool v0.11 home after route visit',async({page})=>{
    await go(page,'duplexscan');await page.goto(`${BASE}/#home`,{waitUntil:'load'});await page.waitForFunction(()=>document.querySelectorAll('[data-tool-card]').length===53,null,{timeout:30000});
    await expect(page.getByText('53 công cụ hoạt động')).toBeVisible();for(const key of ['duplexscan','keywordpages','formfill'])await expect(page.locator(`[href="#${key}"]`)).toHaveCount(1);
  });
